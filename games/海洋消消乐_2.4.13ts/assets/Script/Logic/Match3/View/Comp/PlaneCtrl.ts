...
export default class PlaneCtrl extends cc.Component {
    ...
    public shootPlane(from: cc.Vec2, to: cc.Vec2, delay: number = 0, isCounterClockWise: boolean = false): Promise<any> {
        this.particle.active = false;
        return new Promise((resolve, reject) => {
            from = Common.getPos(from.x, from.y);
            to = Common.getPos(to.x, to.y);
            ...
            arg.node = this.node;
            let startPoint = this.node.position;
            let endPoint = to;
            let s = GapTime.BeikeSuctionSpeed();
    ...
    private getPlaneAngle(pa: cc.Vec3, pc: cc.Vec2, r: number) {
        let angle = 0;
        let x = 0;
        let y = 0;
        if (pa.x < pc.x) {
    ...
    private updateFlyPlane(dt) {
        ...
        if (this._arg.delay > 0) {
            ...
            if (this._arg.move_angle <= 0) {
                ...
                let curX = xc + r * Math.sin(this._arg.angle / 180 * Math.PI);
                let curY = yc + (r * Math.cos(this._arg.angle / 180 * Math.PI));
    ...
    private planeArrive(arg: FlyPlaneArgs) {
        ...
        M.nodePool.freeItem(NodePoolKey.Plane, this.node);
        if (arg.resolve) {
            arg.resolve();
            arg.resolve = null;
        }
